...
const db = new sqlite3.Database('swaps.db');
...
app.get('/api/logs', async (req, res) => {
  try {
    const logContent = await fs.readFile('trading-bot.log', 'utf8');
...
app.get('/api/positions', async (req, res) => {
  try {
    const data = await fs.readFile('trading-bot.log', 'utf8');
    res.json(data);
  } catch (error) {
...
// Original Trading Bot Code (unchanged)
...
class TradingBot {
  ...
  async saveSwapDetails(walletPublicKey, symbol, mintAddress) {
    return new Promise((resolve, reject) => {
        const stmt = db.prepare('INSERT INTO swaps (publickey, symbol, mintAddress) VALUES (?, ?, ?)');
        stmt.run([walletPublicKey, symbol, mintAddress], (err) => {
            if (err) {
  ...
  async performSwap(token, isBuy) {
    this.saveSwapDetails(this.keypair.publicKey.toBase58(), token.token.symbol, token.token.mint);
    logger.info(
...
// Start both the trading bot and the server
